...
const fetchBtn   = document.querySelector("#fetch-btn");     // in your HTML
...
function renderFavs() {
  ...
  list.forEach(icao => {
    ...
    li.querySelector(".chip__del").addEventListener("click", () => removeFav(icao));
    host.appendChild(li);
  });
}

// Alt-click Refresh to add favorite quickly
btnRefresh?.addEventListener("click", (e) => {
  if (e.altKey) {
    const icao = (input?.value || document.querySelector(".icao").textContent).toUpperCase();
    if (isValidIcao(icao)) addFav(icao);
    return; // don't fetch on Alt-click
  }
  // normal click handled later; we keep this for the Alt shortcut only
});

// After a successful fetch, auto-add to favorites (optional)
...
// -----------------------------
// Live fetch via Netlify Function
// -----------------------------
...
function parseMetar(m) {
  ...
  const visM  = m.match(/\b(\d{4})\b/); // crude fallback
  ...
  const parseSigned = s => (s?.startsWith("M") ? -parseInt(s.slice(1), 10) : parseInt(s, 10));
  if (t) {
...
function updateDecodedFromMetar(metar) {
  ...
  const tiles = document.querySelectorAll("#decoded-grid-ok .grid__item .value");
  ...
  if (tiles[4]) tiles[4].innerHTML   = parsed.alt
    ? parsed.alt.replace("inHg", `<abbr title="inches of mercury">inHg</abbr>`)
    : "—";
}
...
async function refreshMetar(passedIcao) {
  console.log("refreshMetar()", { passedIcao });
  const headerIcao = document.querySelector(".icao").textContent.trim().toUpperCase();
  const candidate  = (passedIcao || input?.value || headerIcao || "").toUpperCase();
  ...
  // Show the requested ICAO in the header right away
  ...
  try {
    // Try live
    const metar = await fetchMetar(candidate);
    ...
    console.warn("Live fetch failed, falling back to mock:", err?.message || err);
    // Fallback to mock
    try {
...
// Extra safety: explicit click handler on the submit button
...
// Regular Refresh button (ghost)
btnRefresh?.addEventListener("click", (e) => {
  // Alt+Click handled above to add favorite, so only fetch if no Alt
  if (!e.altKey) refreshMetar();
});
